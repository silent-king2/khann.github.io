...
export default async function handler(req, res) {
  ...
  if (!siteName || !htmlContent) return res.status(400).json({ error: "Missing siteName or htmlContent" });

  const VERCEL_TOKEN = process.env.VERCEL_TOKEN;
  const TELEGRAM_BOT_TOKEN = process.env.TELEGRAM_BOT_TOKEN;
  const TELEGRAM_CHAT_ID = process.env.TELEGRAM_CHAT_ID;

  if (!VERCEL_TOKEN) return res.status(500).json({ error: "Server misconfigured: VERCEL_TOKEN missing" });
  ...
  try {
    ...
    // 2. Buat project (abaikan kalau sudah ada)
